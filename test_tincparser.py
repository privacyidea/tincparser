...
class TestTincUp(unittest.TestCase):
    def test_01_read(self):
        ...
        self.assertEqual(script.prolog, [
            '#!/bin/sh',
            'echo "hello world"'
        ])
        self.assertEqual(script.appliance_section, [
            'ip link set $INTERFACE up',
            'ip addr add 172.20.1.1 dev $INTERFACE',
            'ip route add 172.20.1.0/30 dev $INTERFACE'
        ])
        self.assertEqual(script.epilog, [
            'echo "good bye world"',
            'exit 0'
        ])
    ...
    def test_02_modify(self):
        ...
        script.save()
        with open('testdata/tinc-up', 'r') as f:
    ...
    def test_03_from_scratch(self):
        script = UpScript(LocalIOHandler(), 'testdata/tinc-up2')
        script.appliance_section = ['echo hi']
        script.save()
        with open('testdata/tinc-up2', 'r') as f:
            self.assertEqual(f.read().split('\n'), [
                '#!/bin/sh',
                '# BEGIN AUTOGENERATED',
                'echo hi',
...
class TestNetsBoot(unittest.TestCase):
    def test_01_read(self):
        nets = NetsBoot(LocalIOHandler(), 'testdata/nets.boot')
        self.assertEqual(nets.nets, ['privacyideaVPN'])
    ...
    def test_02_modify(self):
        ...
        nets.save()
        with open('testdata/nets.boot', 'r') as f:
            self.assertEqual(f.read().split('\n'), [
